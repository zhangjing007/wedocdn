...
CoordPicker = /*#__PURE__*/function () {
  ...
  _createClass(CoordPicker, [{ key: "setSelectionPlane", value: function setSelectionPlane(plane) {
    ...
    event) {
      ...
      var hit = this.getPointUnderMouse(event);
      this.fireEvent({ type: Events.POINT_HOVERED, point: hit });

      return this.isDragging;
    }

    // Returns hitPoint under mouse or null if no object under mouse.
  }, { key: "getPointUnderMouse", value: function getPointUnderMouse(event) {

      // Just do selections on the plane if one is specified
      if (this.selectionPlane) {
        ...
        var ray = this.viewer.impl.viewportToRay(vpVec);

        // intersect ray with plane
        return ray.intersectPlane(this.selectionPlane);
      }
      ...
      if (!this.snapperActive) {
        var result = this.viewer.impl.hitTest(event.canvasX, event.canvasY);
        hit = result && result.point;
      } else if (this.snapper.isSnapped()) {
        // Get snapped positoon
        var res = this.snapper.getSnapResult();
        hit = Autodesk.Viewing.MeasureCommon.getSnapResultPosition(res, this.viewer);
      }
    ...
    event, button) {
      ...
      this.fireEvent({ type: Events.POINT_CLICKED, point: hit });
...
var NPointPicker = /*#__PURE__*/function () {
  function NPointPicker(viewer, coordPicker, screenOverlay, N) {var _this = this;var options = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};_classCallCheck(this, NPointPicker);
    ...
    i) {
      ...
      marker.setColor(color);};for (var i = 0; i < this.N; i++) {_loop(i);
    }
