...
StringExtractor = /*#__PURE__*/function (_av$Extension) {_inherits(StringExtractor, _av$Extension);var _super = _createSuper(StringExtractor);
  function StringExtractor(viewer) {var _this;_classCallCheck(this, StringExtractor);
  ...
  }_createClass(StringExtractor, [{ key: "load", value: function load()

    {
      // Since content of Viewer might change, as while switching sheets, string extraction repeated on every model load.
      this.viewer.addEventListener(av.MODEL_ADDED_EVENT, this.extractStringsFromModels);
      return true;
    } }, { key: "unload", value: function unload()

    {
      this.viewer.removeEventListener(av.MODEL_ADDED_EVENT, this.extractStringsFromModels);
      return true;
    } }, { key: "extractStringsFromModels",
    ...
    /**
                                             ...
                                             */value: function extractStringsFromModels()
    {
    ...
    model) {var _this2 = this;
      ...
      if (model.isPdf()) {
        return _StringHelpers__WEBPACK_IMPORTED_MODULE_0__["default"].loadVectorPdfStrings(model).then(function (result) {
          _this2.documentStrings[model.id] = {
            model: model,
            strings: _StringHelpers__WEBPACK_IMPORTED_MODULE_0__["default"].formatVectorPdfStrings(result) };
      ...
      {
        var f2dStrings = _StringHelpers__WEBPACK_IMPORTED_MODULE_0__["default"].loadF2dStrings(model.getData());
...
/**
    * Performs string extraction from Vector PDF file based on PDF.js information
    * @returns {Object} return JSON object with all strings in Vector PDF document.
    */
function loadVectorPdfStrings(model) {
  var strings = [];
  if (typeof model.getData().getPDF === "function") {
    var pageNumber = model.getData().metadata.currentPage;
    var pdf = model.getData().getPDF();

    return pdf.getPage(pageNumber).then(function (page) {
      var readableStream = page.streamTextContent({ normalizeWhitespace: true, combineTextItems: true });
      return new Promise(function (resolve, reject) {
        var pump = function pump() {
          reader.read().then(function (_ref) {var value = _ref.value,done = _ref.done;
            if (done) {
              resolve(strings);
              return;
            }
  ...
  return Promise.reject("Fetching of strings failed since model has no getPDF() function.");
}

/**
   * Format output from loadVectorPdfStrings
   * @param {Object} vectorStrings - Object with all strings from Vector PDF.
   * @returns {Array} formattedStrings - Array with String objects. Each object contains text and bounding box as THREE.Box2.
   */
function formatVectorPdfStrings(vectorStrings) {
  var formattedStrings = [];
  ...
  for (var i = 0; i < vectorStrings.length; i++) {
    if (vectorStrings[i].items) {
      ...
      for (var j = 0; j < items.length; j++) {
        ...
        var minX = tempObject.transform[5];
        var minY = tempObject.transform[4];
        var maxX = tempObject.width + tempObject.transform[5];
        var maxY = tempObject.transform[4] - tempObject.height;
...
var StringHelpers = {
  loadF2dStrings: loadF2dStrings,
  loadLeafletStrings: loadLeafletStrings,
  loadVectorPdfStrings: loadVectorPdfStrings,
  formatleafletStrings: formatleafletStrings,
  formatVectorPdfStrings: formatVectorPdfStrings };
