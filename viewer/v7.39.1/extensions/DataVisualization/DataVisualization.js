...
/*!************************************************************!*\
  !*** ./extensions/DataVisualization/ModelStructureInfo.js ***!
  \************************************************************/
/*! exports provided: LevelRoomsMap, ModelStructureInfo */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
Room = /*#__PURE__*/function () {
  ...
  /**
    ...
    /**
                                                * Adds a {@link Device} to the {@link Room}
                                                *
                ...
                levelName) {
                  ...
                  rooms.forEach(function (room) {
                    ...
                    room.devices.forEach(function (device) {
                      ...
                      node.addPoint(
                      new _heatmap_SurfaceShadingData__WEBPACK_IMPORTED_MODULE_0__["SurfaceShadingPoint"](device.id, device.position, device.sensorTypes, device.name, { styleId: styleId }));
    ...
    {var _this2 = this;
      ...
      this.handleMouseMove = function (event) {
        if (!_this2.buttonDown) {
          var vpVec = viewerImpl.clientToViewport(event.canvasX, event.canvasY);
          var res = [];
          var dbId = viewerImpl.renderer().idAtPixel(vpVec.x, vpVec.y, res, [viewerImpl.renderer().getOverlayIdTarget()]);

          if (_this2.hoveringDbId != dbId && dbId != 0) {
      ...
      this.markObject = function (dbIds) {
        ...
        var geometries = new Set();var
        dbId2Geometry = _this2.delegate.dbId2Geometry;

        // check whether there is viewables in the scene
        if (dbId2Geometry == undefined) {
          return;
        }

        dbIds.map(function (id) {return geometries.add(dbId2Geometry[id]);});
        ...
        geometries.map(function (geometry) {
          ...
          if (geometry && geometry.attributes["id"] && geometry.attributes["flagp"]) {
            ...
            for (var i = 0; i < ids.length; i += 3) {
              ...
              var uv = _this2.delegate.viewableData.getViewableUV(id, flags[index] == 1);
              if (uv) {
                uvp[index * 4] = uv.x;
                uvp[index * 4 + 1] = uv.y;
                uvp[index * 4 + 2] = uv.w;
              ...
              var color = _this2.delegate.viewableData.getViewableColor(id, flags[index] == 1);
              if (color) {
      ...
      this.clearMarkedObject = function (fragments) {
        ...
        if (fragments) {
          fragments.map(function (geometry) {
            // here we need to update the attribute of the geometry
            if (geometry && geometry.attributes["flagp"]) {
              var flags = geometry.attributes["flagp"].array;
              var uvp = geometry.attributes["uvp"].array;
              var colors = geometry.attributes["color"].array;
              var ids = geometry.attributes["id"].array;
      ...
      /**
          * See DataVisualization.invalidateViewables for details.
          * @param {number|number[]} dbIds
          * @param {ViewableInvalidatedCallback} callback
          */
      this.invalidateViewables = function (dbIds, callback) {
        if (!dbIds || !callback) {
        ...
        var fragmentList = _this2.sensorModel.getInstanceTree().fragList;
        var geometryList = fragmentList.geoms.geoms;

        var fragments = new Set();var
        dbId2fragId = fragmentList.fragments.dbId2fragId;
        ...
        var data = _this2.sensorModel.getData().fragments.viewableData;
        data.viewables.forEach(function (viewable) {return viewables.set(viewable.dbId, viewable);});

        var sceneUpdated = false;
        fragments.map(function (fragment) {
          var geometry = geometryList[fragment];
          var ids = geometry.attributes["id"].array;
          ...
          for (var i = 0; i < ids.length; i += 3) {
            ...
            if (updates.url) {
              var uv = data.getSpriteUV(updates.url);
              if (uv) {
                var uvp = geometry.attributes["uvp"].array;
                geometry.attributes["uvp"].needsUpdate = true;
            ...
            // 'highlighted' can be 'false', so test against 'undefined'.
            if (updates.highlighted !== undefined) {
              var flags = geometry.attributes["flagp"].array;
              geometry.attributes["flagp"].needsUpdate = true;
              flags[pointIndex] = updates.highlighted ? 1 : 0;
...
HeatmapMaterial = /*#__PURE__*/function (_THREE$ShaderMaterial) {_inherits(HeatmapMaterial, _THREE$ShaderMaterial);var _super = _createSuper(HeatmapMaterial);
  function HeatmapMaterial(sensorVals, colors, stops, useRooms, confidence) {var alpha = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 1.0;_classCallCheck(this, HeatmapMaterial);
    var uniforms = {
...
/* harmony import */ var _heatmap_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./heatmap.js */ "./extensions/DataVisualization/heatmap/heatmap.js");
/* harmony import */ var _heatmap_js__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_heatmap_js__WEBPACK_IMPORTED_MODULE_0__);
...
PlanarHeatmap = /*#__PURE__*/function () {
  function PlanarHeatmap(viewer, model, shadingData) {var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};_classCallCheck(this, PlanarHeatmap);
    options = Object.assign(
    {
      ...
      height: 1024,
      slicingEnabled: true,
      position: "middle", // {min|middle|max},
      placePosition: "min" // {min|max}
    },
    options);var _options =


    options,width = _options.width,height = _options.height;
  ...
  /**
     ...
     * @param {number} radius
     */_createClass(PlanarHeatmap, [{ key: "_setup", value: function _setup(
    nodeIds, sensorType) {var radius = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 200;
      ...
      this.heatmapInstance = _heatmap_js__WEBPACK_IMPORTED_MODULE_0___default.a.create(this.config);
      ...
      var zPosition = this.options.position;
      var placePosition = this.options.placePosition;
      var frags = model.getFragmentList();

      for (var _i = 0; _i < leafs.length; _i++) {
        var currentLeafNode = leafs[_i];
        // only add those with shading points
        if (!isLeafContainsSensor(currentLeafNode)) {
          continue;
        }
        ...
        if (slicingEnabled) {var
          ...
          intersects;(function () {var normal = new THREE.Vector3(0, 0, 1); //Create the 2D line geometry
            ...
            intersects = [];currentLeafNode.dbIds.forEach(function (dbId) {var bounds = boundsMap[dbId];
              var zConstant = (bounds.min.z + bounds.max.z) / 2;
              if (zPosition == "min") {
                zConstant = bounds.min.z;
              } else if (zPosition == "max") {
                zConstant = bounds.max.z;
              }

              var zValue = bounds[placePosition].z;

              var plane = new THREE.Plane(normal, -zConstant);
          ...
          currentLeafNode.dbIds.forEach(function (dbId) {
            ...
            if (!bounds.empty()) {var
              min = bounds.min,max = bounds.max,zPlane = bounds[zPosition];

              var index = positions.length / 3;

              indices.push(index, index + 2, index + 1, index, index + 3, index + 2);
              positions.push(
              min.x, min.y, zPlane.z,
              min.x, max.y, zPlane.z,
              max.x, max.y, zPlane.z,
              max.x, min.y, zPlane.z);
        ...
        // shading points
        var _iterator = _createForOfIteratorHelper(currentLeafNode.shadingPoints),_step;try {for (_iterator.s(); !(_step = _iterator.n()).done;) {var point = _step.value;
            var pData = {
    ...
    nodeIds, sensorType, sensorValueCallback) {var confidenceSize = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 200;
      if (this.geometry) {
    ...
    sensorValueCallback) {
      for (var key in this.pointDataMap) {
        ...
        point.data.value = sensorValueCallback(point.shadingPoint, this.currentSensorType, point.data);
      }
    ...
    sensorType, colors) {
      ...
      for (var i = 0; i < colors.length; i++) {
        gradient[i / size] = intToRGB(colors[i]);
      }
...
SurfaceShading = /*#__PURE__*/function () {
  ...
  /**
    ...
    /**
       ...
       * @param {number} confidenceSize The confidence size.
       */ }, { key: "render", value: function render(
    nodeId, sensorType, sensorValueCallback) {var _this = this;var confidenceSize = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 160;
...
/***/ "./extensions/DataVisualization/heatmap/heatmap.js":
/*!*********************************************************!*\
  !*** ./extensions/DataVisualization/heatmap/heatmap.js ***!
  \*********************************************************/
/*! no static exports found */
...
/* harmony import */ var _heatmap_SurfaceShadingData__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./heatmap/SurfaceShadingData */ "./extensions/DataVisualization/heatmap/SurfaceShadingData.js");
/* harmony import */ var _heatmap_SurfaceShadingData__WEBPACK_IMPORTED_MODULE_8___default = /*#__PURE__*/__webpack_require__.n(_heatmap_SurfaceShadingData__WEBPACK_IMPORTED_MODULE_8__);
...
ns = Object.assign(ns, _heatmap_SurfaceShadingData__WEBPACK_IMPORTED_MODULE_8__);
...
/**
                                            ...
                                            * @property {boolean} [highlighted] An optional property that determines if
                                            * the `CustomViewable` should be highlighted or not.
...
DataVisualization = /*#__PURE__*/function (_av$Extension) {_inherits(DataVisualization, _av$Extension);var _super = _createSuper(DataVisualization);
  ...
  function DataVisualization(viewer) {var _this;var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};_classCallCheck(this, DataVisualization);
    ...
    if (options["useInternal"] !== true) {
      ...
      console.warn(terms);
  ...
  }_createClass(DataVisualization, [{ key: "load", value: function () {var _load = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {var options;return regeneratorRuntime.wrap(function _callee$(_context) {while (1) {switch (_context.prev = _context.next) {case 0:
                ...
                this.datavizDotOverlay = this.viewer.impl.createOverlayScene(datavizDotOverlay, null, null, null, true, true);
    ...
    data) {var _this2 = this;
      ...
      var depthTarget = this.viewer.impl.renderer().getDepthTarget();

      // When AmbientOclution is off, there is no depthTarget
      if (depthTarget) {
        material.uniforms = {
          "tDepthTest": { type: "t", value: depthTarget },
          "tDepthResolution": { type: "v2", value: new THREE.Vector2(1 / depthTarget.width, 1 / depthTarget.height) } };
      ...
      matman.addMaterial("point1", material, true);
      ...
      this.viewer.addEventListener(av.VIEWER_RESIZE_EVENT, function () {
        ...
        var depthTarget = _this2.viewer.impl.renderer().getDepthTarget();
        if (depthTarget) {
          material.uniforms["tDepthTest"].value = depthTarget;
          material.uniforms["tDepthResolution"].value.set(1 / depthTarget.width, 1 / depthTarget.height);
        }
      });
    ...
    /**
       ...
       *
       * @throws {Error} Both the parameters are mandatory for this function.
    ...
    /**
       ...
       * The current implementation ignores this parameter unless `options.type` is
       * set to `"PlanarHeatmap"`.
       ...
       * @param {string} [options.position] Optional. A `string` value representing
       * the point at which slicing should happen. Valid values include `min`, `middle`
       * and `max`. If unspecified, the default value of `middle` will be used, slicing
       * geometry through the vertical mid-point of the bounding box.
       * @param {string} [options.placePosition] Optional. A `string` value that
       * can either be `min` or `max`, indicating the placement position of the
       * resulting plane. When set to `min`, the plane will be positioned at the
       * lowest `z` coordinate of the bounding box. When set to `max`, the plane will
       * be positioned at the highest point of the bounding box. If unspecified, the
       * default value of `min` will be used.
       *
       * @example
       *  // 'devices' is a list of 'Device' objects
       *  const info = new Autodesk.DataVisualization.Core.ModelStructureInfo(model);
       ...
       */ }, { key: "setupSurfaceShading", value: function () {var _setupSurfaceShading = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(
      model, shadingData) {var options,_args2 = arguments;return regeneratorRuntime.wrap(function _callee2$(_context2) {while (1) {switch (_context2.prev = _context2.next) {case 0:options = _args2.length > 2 && _args2[2] !== undefined ? _args2[2] : {};
                if (this.surfaceShading) {
                  this.surfaceShading.cleanUp();
                }if (!(

                options.type == "PlanarHeatmap")) {_context2.next = 8;break;}_context2.next = 5;return (
                  this.viewer.loadExtension('Autodesk.CompGeom'));case 5:
                this.surfaceShading = new _heatmap_PlanarHeatmap__WEBPACK_IMPORTED_MODULE_6__["PlanarHeatmap"](this.viewer, model, shadingData, options);_context2.next = 9;break;case 8:

                this.surfaceShading = new _heatmap_SurfaceShading__WEBPACK_IMPORTED_MODULE_5__["SurfaceShading"](this.viewer, model, shadingData);case 9:case "end":return _context2.stop();}}}, _callee2, this);}));function setupSurfaceShading(_x, _x2) {return _setupSurfaceShading.apply(this, arguments);}return setupSurfaceShading;}()
