...
ViewCubeUi.prototype = {
  ...
  load: function load() {
    ...
    if (this.viewer.model) {
      this._onModelAdded({ model: this.viewer.model });
    }

    // Register a event handler to update the view cube (not just once for for model unloading/loading use cases)
  ...
  _updateView: function _updateView(viewType) {
    if (this.viewer.impl.is2d) return;
    var storedView = this.viewer.prefs.get(avp.Prefs3D.VIEW_TYPE);
    // Do not set the preference if the viewType
    if (viewType === storedView) return;
    this.viewer.prefs.set(avp.Prefs3D.VIEW_TYPE, viewType);
  },
  ...
  /**
      * @private
      */
  _initCompass: function _initCompass(model) {
    ...
    var radians = projectNorthAngle + data.loadOptions && data.loadOptions.applyRefPoint ? 0 : trueNorthAngle;
  ...
  _onModelAdded: function _onModelAdded(event) {
    // As soon as a 2D model is loaded, we hide the view cube (assuming only one 2D model at a time is loaded)
    this._displayViewCube(event.model.is2d() && !event.isOverlay);

    if (this.cube && !this.cube.hasCompass) {
