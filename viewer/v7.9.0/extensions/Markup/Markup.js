...
proto.onMouseMove = function (event) {
  ...
  if (editor.snapper && !editor.snapper.isSnapped() && this.lineSnapped) {
    ...
    var p = new THREE.Vector3(_final.x, _final.y, start.z);
    ...
    // Snap to perpendicular of underlying vectors
        ...
        _final.x = perpendicular.x;
        _final.y = perpendicular.y;
      }
  }
  ...
  this.size = editor.sizeFromClientToMarkups(_final.x - initialX, _final.y - initialY);
...
proto.onMouseMove = function (event) {
  ...
  var _final = this.getFinalMouseDraggingPosition();
  _final = editor.clientToMarkups(_final.x, _final.y);

  var sizeX = Math.abs(this.firstPosition.x - _final.x);
  var sizeY = Math.abs(this.firstPosition.y - _final.y);

  var position = { x: (this.firstPosition.x + _final.x) * 0.5, y: (this.firstPosition.y + _final.y) * 0.5 };
...
proto.onMouseMove = function (event) {
  ...
  var _final = editor.clientToMarkups(pos.x, pos.y);
  var position = { x: (this.firstPosition.x + _final.x) / 2, y: (this.firstPosition.y + _final.y) / 2 };
  var size = this.size = { x: Math.abs(_final.x - this.firstPosition.x), y: Math.abs(_final.y - this.firstPosition.y) };
...
proto.onMouseMove = function (event) {
  ...
  var pos = this.getFinalMouseDraggingPosition();
  var _final = editor.clientToMarkups(pos.x, pos.y);
  var position = { x: (this.firstCorner.x + _final.x) / 2, y: (this.firstCorner.y + _final.y) / 2 };
  ...
  if (editor.snapper && this.lineSnapped) {
    ...
    var p = new THREE.Vector3(_final.x, _final.y, startZ);
  ...
  {
    width = _final.x - this.firstCorner.x;
    height = _final.y - this.firstCorner.y;
  }
